...
function Settings(props) {
  ...
  const [showMessages, setShowMessages] = useState(
    storedSettings ? storedSettings.showMessages : false
  );
  const [showNewContent, setShowNewContent] = useState(
    storedSettings ? storedSettings.showNewContent : false
  );
  const [nightMode, setNightMode] = useState(
    storedSettings ? storedSettings.nightMode : true
  );
  const [highContrast, setHightContrast] = useState(
    storedSettings ? storedSettings.highContrast : false
  );
  const [customBackground, setCustomBackground] = useState(
    storedSettings ? storedSettings.customBackground : false
  );
  const [agreementData, setAgreementData] = useState(
    storedSettings ? storedSettings.agreementData : false
  );
  const [agreementInfo, setAgreementInfo] = useState(
    storedSettings ? storedSettings.agreementInfo : false
  );
  const [agreementTerms, setAgreementTerms] = useState(
    storedSettings ? storedSettings.agreementTerms : false
  );

  const [color, setColor] = useState(
    storedSettings ? storedSettings.color : [0, 0, 0]
  );
  const [hue, setHue] = useState(storedSettings ? storedSettings.color[0] : 0);
  const [saturation, setSaturation] = useState(
    storedSettings ? storedSettings.color[1] : 0
  );
  const [lightness, setLightness] = useState(
    storedSettings ? storedSettings.color[2] : 0
  );
  ...
  return (
    <article className={`settings ${slidein ? "slide-card" : ""}`}>
        ...
        <li className="settings__list-item">
          ...
          <div className="settings__toggle">
            <label>
              <input
                type="checkbox"
                checked={settings.showNewContent}
                onChange={() => setShowNewContent(!showNewContent)}
              />
              <div className="settings__toggle-mark"></div>
              <div className="settings__toggle-bg"></div>
            </label>
      ...
      <ul className="settings__list">
          ...
          <div className="settings__toggle">
            <label>
              <input
                ...
                onChange={() => setHightContrast(!highContrast)}
              />
              <div className="settings__toggle-mark"></div>
              <div className="settings__toggle-bg"></div>
            </label>
        ...
        <li className="settings__list-item">
          ...
          <div className="settings__toggle">
            <label>
              <input
                type="checkbox"
                checked={settings.customBackground}
        ...
        <div
          className={`settings__slider ${!customBackground ? "disabled" : ""}`}
        >
      ...
      <div className="settings__list">
        ...
        <div className="settings__list-agreement">
          <label>
            <input
              type="checkbox"
              checked={settings.agreementData}
        ...
        <div className="settings__list-agreement">
          <label>
            <input
              type="checkbox"
              checked={settings.agreementInfo}
              onChange={() => setAgreementInfo(!agreementInfo)}
              className="agreements"
            />
          </label>
        ...
        <div className="settings__list-agreement">
          <label>
            <input
              type="checkbox"
              checked={settings.agreementTerms}
              onChange={() => setAgreementTerms(!agreementTerms)}
              className="agreements"
            />
          </label>
      ...
      <div className="settings__list">
        <button className="settings__list-reset" onClick={handleSettingsReset}>
          Reset all settings
        </button>
      </div>
    </article>
